...
import {
  messageHeaderJson,
  messageJson, scenarioActionJson,
  scenarioExecutionJsonWithDetails,
  scenarioExecutionJsonWithoutDetails, twoScenarioExecutions
...
test('should filter with input form', async ({page}) => {
  await page.goto('http://localhost:9000/scenario-result/');
  mockBackendResponse(page,'**/api/scenario-executions?page=0&size=10&sort=executionId,asc', twoScenarioExecutions)
  mockBackendResponse(page,'**/api/scenario-executions?page=1&size=10&sort=executionId,asc&filter%5BstartDate.greaterThanOrEqual%5D=2024-07-21T10:05:31.000Z&filter%5BendDate.lessThanOrEqual%5D=2024-07-22T03:02:07.000Z', [])
