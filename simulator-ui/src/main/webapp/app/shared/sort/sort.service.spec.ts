...
import { SortOrder } from './sort-state';
...
describe('sort state', () => {
  ...
  describe('parseSortParam', () => {
    ...
    it('should accept predicate and ASC string', () => {
      ...
      expect(sortState).toEqual({ predicate: 'predicate', order: SortOrder.ASCENDING });
    });
    it('should accept predicate and DESC string', () => {
      const sortState = service.parseSortParam('predicate,desc');
      expect(sortState).toEqual({ predicate: 'predicate', order: SortOrder.DESCENDING });
    });
  });
  describe('buildSortParam', () => {
    ...
    it('should accept object with predicate and asc value', () => {
      const sortParam = service.buildSortParam({ predicate: 'column', order: SortOrder.ASCENDING });
      expect(sortParam).toEqual(['column,asc']);
    });
    it('should accept object with predicate and desc value', () => {
      const sortParam = service.buildSortParam({ predicate: 'column', order: SortOrder.DESCENDING });
