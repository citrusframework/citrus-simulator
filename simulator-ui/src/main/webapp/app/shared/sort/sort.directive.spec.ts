...
describe('Directive: SortDirective', () => {
  ...
  it('should invoke sortChange function', () => {
    ...
    expect(component.transition).toHaveBeenCalledWith({ predicate: 'ID', order: SortOrder.ASCENDING });
  });
  ...
  it('should change sort order to descending, neutral when same field is sorted again', () => {
    ...
    expect(component.transition).toHaveBeenNthCalledWith(1, { predicate: 'ID', order: SortOrder.ASCENDING });
    expect(component.transition).toHaveBeenNthCalledWith(2, { predicate: 'ID', order: SortOrder.DESCENDING });
    expect(component.transition).toHaveBeenNthCalledWith(3, { predicate: 'ID', order: SortOrder.ASCENDING });
  });
  ...
  it('should change sort order to ascending when different field is sorted', () => {
    ...
    expect(component.transition).toHaveBeenNthCalledWith(1, { predicate: 'ID', order: SortOrder.ASCENDING });
    expect(component.transition).toHaveBeenNthCalledWith(2, { predicate: 'NAME', order: SortOrder.ASCENDING });
  });
});
