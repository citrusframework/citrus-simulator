...
export class AlertError implements OnDestroy {
  ...
  constructor() {
    this.errorListener = this.eventManager.subscribe('citrusSimulatorApp.error', (response: EventWithContent<unknown> | string) => {
      const errorResponse = (response as EventWithContent<AlertErrorModel>).content;
      this.addErrorAlert(errorResponse.message, errorResponse.key, errorResponse.params);
    });

    this.httpErrorListener = this.eventManager.subscribe('citrusSimulatorApp.httpError', (response: EventWithContent<unknown> | string) => {
  ...
  private addErrorAlert(message?: string, translationKey?: string, translationParams?: Record<string, unknown>): void {
    const extAlerts: AlertModel[] = [];
    const alert = this.alertService.addAlert({ type: 'danger', message, translationKey, translationParams }, extAlerts);
  ...
  private handleFieldsError(httpErrorResponse: HttpErrorResponse): void {
    ...
    for (const fieldError of fieldErrors) {
      ...
      const fieldName: string = this.translateService.instant(`citrusSimulatorApp.${fieldError.objectName as string}.${convertedField}`);
