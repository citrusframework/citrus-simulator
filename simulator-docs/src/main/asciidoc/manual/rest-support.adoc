...
*Note*
The REST support is using a different default mapping key extractor. The *HttpRequestAnnotationScenarioMapper* is active by default and enables *@RequestMapping*
...
The *HttpRequestAnnotationScenarioMapper* is active by default so in case you need to apply different mapping strategies you must overwrite the mapping key extractor
