...
public class DynamicClassLoader {
    ...
    public static <T> Class<T> compileAndLoad(String className, String sourceCodeInText) throws Exception {
        var source = new InMemoryJavaFileObject(className, sourceCodeInText);
        Iterable<? extends JavaFileObject> compilationUnits = singletonList(source);

        var inMemoryJavaFileManager = new InMemoryJavaFileManager(STD_FILE_MANAGER);

        var compilationTask = JAVA_COMPILER.getTask(null, inMemoryJavaFileManager, null, null, null, compilationUnits);
